"use client";
import { homeInteriorPageClientReviewApi } from "@/axios/axios";
...
interface HomeClientReviewType {
  ms: number;
  query: string;
  result: {
    _id: string;
...
const fetchHomeInteriorPageClientReview = async () => {
  try {
    const res = await homeInteriorPageClientReviewApi();
    if (res.status !== 200) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error("HomeInteriorClientReviewFetchError", error.message);
    }
    return;
  }
};
...
const HomeCustomerReview = () => {
  const HomeInteriorClientReviewQuery = useSuspenseQuery<HomeClientReviewType>({
    queryKey: ["home-interior-review"],
    queryFn: fetchHomeInteriorPageClientReview,
  });
  const { result } = HomeInteriorClientReviewQuery.data;
  if (process.env.NODE_ENV === "development") {
